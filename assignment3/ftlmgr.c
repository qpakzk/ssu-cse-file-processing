...
void ftl_write(int lsn, char *sectorbuf)
{
	...
	if(lsn < 0 || lsn >= DATABLKS_PER_DEVICE * PAGES_PER_BLOCK) {
		fprintf(stderr, "Logical Sector Number %d is out of range.\n", lsn);
		exit(1);
	}

	lbn = lsn / SECTORS_PER_BLOCK; 
